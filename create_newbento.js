...
async function run() {
    const name = "New Bento";
    const phone = "+998938478887";
    const passwordPlain = "bento";
    ...
    if (existingUser) {
        ...
        await supabase.from('users').update({ password: passwordHash, name: name }).eq('id', userId);
    } else {
        userId = "user_" + Math.random().toString(36).substr(2, 9);
        const { error } = await supabase.from('users').insert({
            id: userId, phone: phone, username: phone, name: name,
    ...
    if (!tariff) { console.error("Tariff 'SALOM 30' not found!"); return; }
    console.log("Found tariff:", tariff.name, tariff.id);

    const promoCode = "BENTO30";
    ...
    if (existingPromo) {
         ...
         const { error: promoErr } = await supabase.from('affiliate_promo_codes').insert({
              affiliate_id: userId, tariff_id: tariff.id, code: promoCode,
              title: "New Bento uchun promokod (Salom 30)",
              is_active: true, usage_limit: 0, usage_count: 0, total_earned: 0
         });
         if (promoErr) { console.error("Error creating promo code:", promoErr); }
         else { console.log("Promo code created:", promoCode); }
    }
}
run();
