...
const baseDir = 'd:/Desktop/aaa/choynaklar';
const kettlesFile = 'd:/Desktop/aaa/choynaklar/kettles.json';
const KETTLES_CAT_ID = '505'; // "Elektr choynaklar (Tefal)"
...
function translateToUzbek(val) {
    ...
    let str = val;
    const replacements = [
        { rx: /Стеклянный|Стекло/g, rep: "Shisha" },
        { rx: /Пластик|Пластиковый/g, rep: "Plastik" },
        { rx: /Металл|Металлический/g, rep: "Metall" },
...
async function run() {
    console.log(`=== STARTING KETTLES IMPORT TO DB ===\n`);
    
    const kettles = JSON.parse(fs.readFileSync(kettlesFile, 'utf-8'));
    
    const localProducts = [];
    for (const [id, prod] of Object.entries(kettles)) {
        localProducts.push({ id, ...prod, categoryId: KETTLES_CAT_ID });
    }
    ...
    const allDirs = fs.readdirSync(baseDir).filter(f => {
        return fs.statSync(path.join(baseDir, f)).isDirectory() && f !== 'categories';
    });
    ...
    for (const lp of localProducts) {
        ...
        const modelName = lp.model || `KT-${lp.id}`;
        ...
        for (const [pName, pValue] of Object.entries(lp.parameters)) {
            if (pValue !== null && pValue !== undefined && pValue !== '') {
                ...
                // Clean Materials
                if (pName === 'Korpus materiali' || pName === 'Material') {
