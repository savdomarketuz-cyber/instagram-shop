        ...
        <header className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-2xl z-[100] border-b border-gray-100 h-16 md:h-24">
            <div className="max-w-[1440px] mx-auto h-full px-4 md:px-10 flex items-center gap-3 md:gap-12">
                ...
                <Link href="/" className="shrink-0 group transition-transform active:scale-95">
                    <Logo size="sm" className="!items-start md:scale-125" />
                </Link>
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-3xl border-t border-gray-100 px-6 py-3 flex justify-between items-center pb-8 z-[100] shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
                <Link href="/" className={`flex flex-col items-center gap-1 ${pathname === '/' ? 'text-black' : 'text-gray-400'}`}>
                    <LayoutGrid size={24} strokeWidth={pathname === '/' ? 3 : 2} />
                    <span className="text-[8px] font-black uppercase tracking-tighter">Asosiy</span>
                </Link>
                <Link href="/reels" className={`flex flex-col items-center gap-1 ${pathname === '/reels' ? 'text-black' : 'text-gray-400'}`}>
                    <Clapperboard size={24} strokeWidth={pathname === '/reels' ? 3 : 2} />
                    <span className="text-[8px] font-black uppercase tracking-tighter">Reels</span>
                </Link>
                <Link href="/cart" className={`relative flex flex-col items-center gap-1 ${pathname === '/cart' ? 'text-black' : 'text-gray-400'}`}>
                    <ShoppingCart size={24} strokeWidth={pathname === '/cart' ? 3 : 2} />
                    ...
                    <span className="text-[8px] font-black uppercase tracking-tighter">Savat</span>
                </Link>
                <Link href="/wishlist" className={`flex flex-col items-center gap-1 ${pathname === '/wishlist' ? 'text-black' : 'text-gray-400'}`}>
                    <Heart size={24} strokeWidth={pathname === '/wishlist' ? 3 : 2} />
                    <span className="text-[8px] font-black uppercase tracking-tighter">Sevimlilar</span>
                </Link>
                <Link href={user ? "/account" : "/login"} className={`flex flex-col items-center gap-1 ${pathname?.includes('/account') ? 'text-black' : 'text-gray-400'}`}>
                    <User size={24} strokeWidth={pathname?.includes('/account') ? 3 : 2} />
