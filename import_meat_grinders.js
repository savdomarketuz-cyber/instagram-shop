...
const grindersFile = 'd:/Desktop/aaa/meat_grinders.json';
const GRINDERS_CAT_ID = '516'; // "Go'shtqiymalagichlar"
...
const PARAM_NAME_MAPPING = {
    ...
    'reverse': 'Revers',
    'reverse_mode': 'Revers',
    'revers_function': 'Revers',
    'reverse_function': 'Revers',
    
    // Productivity
    'meat_grinding_capacity': 'Unumdorlik, kg/min',
    'productivity': 'Unumdorlik, kg/min',
    'meat_processing_capacity': 'Unumdorlik, kg/min',
    'production_capacity': 'Unumdorlik, kg/min',
    'performance': 'Unumdorlik, kg/min',
    'processing_speed': 'Unumdorlik, kg/min',
    
    // Attachments
    'attachments': 'Nasadkalar',
    'nozzles': 'Nasadkalar',
    'included_attachments': 'Nasadkalar',
    'nozzle': 'Nasadkalar',
    ...
    'working_elements_material': 'Material',
...
function translateToUzbek(pName, val) {
    ...
    if (pName === 'Unumdorlik, kg/min') {
        str = str.replace(/\b(\d+(\.\d+)?)\s*(kg\/min|kg\/m|кг\/мин|кг\/м)\b/ig, '$1 kg/min').trim();
        if (/^\d+(\.\d+)?$/.test(str)) str = str + ' kg/min';
    }
...
async function run() {
    console.log(`=== STARTING MEAT GRINDERS IMPORT TO DB (${DRY_RUN ? "DRY RUN MODE" : "LIVE MODE"}) ===\n`);
    
    const grindersData = JSON.parse(fs.readFileSync(grindersFile, 'utf-8'));
    
    const localProducts = [];
    for (const [id, prod] of Object.entries(grindersData)) {
        localProducts.push({ id, ...prod, categoryId: GRINDERS_CAT_ID });
    }
    ...
    for (const lp of localProducts) {
        ...
        const modelName = lp.model || `MG-${lp.id}`;
