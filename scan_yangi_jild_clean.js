const fs = require('fs');
const path = require('path');
const xlsx = require('xlsx');

// Suppress console.error/warn during xlsx read
const originalWarn = console.warn;
const originalError = console.error;
console.warn = () => {};
console.error = () => {};

const dir = 'D:\\Desktop\\Yangi jild';

async function run() {
    const files = fs.readdirSync(dir).filter(f => f.endsWith('.xlsx'));
    const results = [];
    
    for (const f of files) {
        const fullPath = path.join(dir, f);
        try {
            const wb = xlsx.readFile(fullPath, { bookSheets: true });
            const sheetName = wb.SheetNames[2];
            if (sheetName) {
                const wbFull = xlsx.readFile(fullPath);
                const sheet = wbFull.Sheets[sheetName];
                const data = xlsx.utils.sheet_to_json(sheet, {header: 1});
                const firstCell = data[0] ? String(data[0][0]) : "";
                
                let headerRowIndex = -1;
                for(let i=0; i<10; i++) {
                    if(data[i] && data[i].includes('Mahsulot nomi *')) { headerRowIndex = i; break; }
                }
                
                let prodCount = 0;
                if (headerRowIndex !== -1) {
                    const headers = data[headerRowIndex];
                    const nameIdx = headers.indexOf('Mahsulot nomi *');
                    for(let i = headerRowIndex + 2; i < data.length; i++) {
                        const row = data[i];
                        if(row && row[nameIdx] && !String(row[nameIdx]).includes('Agar bir nechta') && !String(row[nameIdx]).includes('Sxemaga e\'tibor')) {
                            prodCount++;
                        }
                    }
                }
                results.push(`${f} | Cat: ${firstCell.replace('Kategoriya: ', '')} | Prods: ${prodCount}`);
            }
        } catch (e) {
            results.push(`${f} | Error: ${e.message}`);
        }
    }
    
    // Restore console
    console.warn = originalWarn;
    console.error = originalError;
    
    fs.writeFileSync('excel_categories.txt', results.join('\n'));
    console.log(`Scan completed. Written to excel_categories.txt. Total files processed: ${files.length}`);
}

run().catch(originalError);
