...
async function run() {
    const name = "Muhammadvali"; 
    const phone = "+998996989334";
    const passwordPlain = "muhammadvali123";
    ...
    // 3. Create Promo Code
    const promoCode = "muhammadvali10";
    ...
    if (existingPromo) {
         ...
         const { error: promoErr } = await supabase.from('affiliate_promo_codes').insert({
              ...
              tariff_id: tariff.id,
              code: promoCode,
              title: "Muhammadvali uchun promokod (Salom 30)",
              is_active: true,
              usage_limit: 0,
              usage_count: 0,
              total_earned: 0
