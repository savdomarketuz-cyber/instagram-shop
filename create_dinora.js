...
async function run() {
  ...
  const { data: exCode } = await supabase.from('affiliate_promo_codes')
    .select('id').eq('code', affiliate.code).single();
  if (exCode) {
    console.log(`  Kod allaqachon bor: ${affiliate.code}`);
  } else {
    ...
    else console.log(`  ✅ Kod yaratildi: ${affiliate.code}`);
  }

  console.log("\n\n===== YAKUN =====");
  console.log(`Login: ${affiliate.phone}`);
  console.log(`Parol: ${affiliate.pwd}`);
  console.log(`Promo-kod: ${affiliate.code}\n`);
}
